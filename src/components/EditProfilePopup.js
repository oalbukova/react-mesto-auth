...
function EditProfilePopup(props) {
  ...
  return (
    <PopupWithForm name="form_profile" title='Редактировать профиль' submit='Сохранить'
                   isOpen={props.isOpen} onClose={props.onClose} onSubmit={handleSubmit} isLoading={props.isLoading} disabled={disabled}>
      <input className="popup__input popup__input_type_name" id="name-input" type="text" required
             placeholder="Имя" value={name || ''} ref={nameRef}
             name="name" pattern="[A-Za-zА-Яа-яЁё -]*" minLength="2" maxLength="40" onChange={handleChangeName} />
      <span className={`popup__span-error ${ !nameValid && 'popup__span-error_type_active'}`} id="name-input-error">{nameError}</span>
      <input className="popup__input popup__input_type_job" id="job-input" type="text" required
             placeholder="О себе" value={description || ''} ref={descriptionRef}
             name="about" minLength="2" maxLength="200" onChange={handleChangeDescription} />
      <span className={`popup__span-error ${ !descriptionValid && 'popup__span-error_type_active'}`} id="job-input-error">{descriptionError}</span>
    </PopupWithForm>
  );
}
